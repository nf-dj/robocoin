...
def generate_randomized_hadamard_matrix(n):
    ...
    multiplies each row by a random sign (+1 or -1), and randomly permutes rows and columns.
...
###############################################################################
# Dense and Orthogonal Matrix Generators
###############################################################################
def generate_dense_rademacher_matrix(n):
    """
    Generates an n x n dense Rademacher matrix (each entry is independently ±1).
    """
    return np.random.choice([1, -1], size=(n, n))

def generate_random_orthogonal_matrix(n):
    """
    Generates an n x n random orthogonal matrix using QR decomposition.
    """
...
def generate_approx_orthogonal_ternary_matrix(n, q, tol, max_attempts):
    """
    Generate an n x n matrix with entries in {-1,0,1} row-by-row using a greedy method.
    A candidate row is accepted if its dot product with every previously accepted row is within ±tol.
    Returns the matrix as a NumPy array if successful, or None otherwise.
    """
    rows = []
    for i in range(n):
        accepted = False
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate an n x n ternary weighting matrix with approximate orthogonality using a greedy method (seeded), then transform random input vectors using multiple rounds of matrix multiplication, bias, scaling, Gaussian noise, and thresholding. Optionally, choose the matrix generation method via --matrix_type: greedy, dense, orthogonal, or hadamard. Finally, check the resulting bitstream using standard randomness tests."
    )
    parser.add_argument("--n", type=int, default=256,
                        help="Matrix size (n x n). Default is 256.")
    parser.add_argument("--q", type=float, default=0.05,
                        help="Probability for each entry to be +1 (and -1) for the greedy method.")
    parser.add_argument("--tol", type=int, default=3,
                        help="Tolerance for dot product between distinct rows (greedy method). Default is 3.")
    parser.add_argument("--max_attempts", type=int, default=100000,
                        help="Maximum candidate attempts per row for the greedy method. Default is 100000.")
    ...
    parser.add_argument("--num_rounds", type=int, default=1,
                        help="Number of rounds of processing per input vector. Default is 1.")
    parser.add_argument("--block_size", type=int, default=2048,
                        help="Block size for block frequency test. Default is 2048.")
    parser.add_argument("--matrix_type", type=str, default="greedy",
                        choices=["greedy", "dense", "orthogonal", "hadamard"],
                        help="Matrix generation method: 'greedy' (default), 'dense', 'orthogonal', or 'hadamard'.")
    ...
    # If using the greedy method, generate a sample matrix and show diagnostics.
    if args.matrix_type == "greedy":
        ...
        print(f"Using {args.matrix_type} matrices for each round (greedy matrix diagnostics skipped).")
    
    # Generate matrices for each round.
    print(f"\nGenerating matrices for {args.num_rounds} rounds using the '{args.matrix_type}' method...")
    round_matrices = []
    round_biases = []
    for r in range(args.num_rounds):
        if args.matrix_type == "dense":
            M_r = np.random.choice([1, -1], size=(n, n))
        elif args.matrix_type == "orthogonal":
            X = np.random.randn(n, n)
            Q, R = np.linalg.qr(X)
            D = np.diag(np.sign(np.diag(R)))
            M_r = Q @ D
        elif args.matrix_type == "hadamard":
            H = hadamard_matrix(n)
            row_signs = np.random.choice([1, -1], size=(n, 1))
            M_r = H * row_signs
            # Optionally, you can permute rows and columns.
            row_perm = np.random.permutation(n)
            col_perm = np.random.permutation(n)
            M_r = M_r[row_perm, :][:, col_perm]
        else:  # greedy
